...
| POWERTOOLS_IGNORE_ERRORS            | true                                                                                        | We internally use Yan Cui's "lambda-powertools-logger" middleware, so this setting disables its error handling, so we can do such ourselves through the Middy Wrap "Essentials" included with the plugin. More information about this can be found in the [Middy Wrap "Essentials"](#middy-wrap-essentials) section |
